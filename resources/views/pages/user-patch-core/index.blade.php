...
<div class="card">
            ...
            <table class="table card-table table-vcenter text-nowrap datatable">
                <thead>
                    <tr>
                        ...
                        <th>
                            <button class="table-sort d-flex justify-content-between desc" data-sort="sort-patch-core">Nama Patch Core</button>
                        </th>
                        <th>
    ...
    <div class="card-footer d-flex align-items-center">
        <p class="m-0 text-secondary">
            Showing <span>{{ $userPatchCore->firstItem() ?? 0 }}</span> 
            to <span>{{ $userPatchCore->lastItem() ?? 0 }}</span> of
            <span>{{ $userPatchCore->total() }}</span> entries
        </p>
...
<script>
    const advancedTable = {
        headers: [
            ...
            { "data-sort": "sort-patch-core", name: "Nama Patch Core" },
...
<script>
    ...
    $("#addBtn").click(function() {
        $(".modal-title").html("Tambah Stock Patch Core");
        $("#role").val("");
        $("#user_id").val("");
        $("#patch_core_id").val("");
        $("#total").val("");
    ...
    $("#storeAddStock").click(function() {
        let user_patch_core_id = $("#user_patch_core_id").val();
        let total_stock = $("#total_stock").val()

        let url = "{{ route('user.patch.core.addStore') }}";
        
        $.ajax({
            url: url,
            method: "POST",
            data: {
                user_patch_core_id: user_patch_core_id,
