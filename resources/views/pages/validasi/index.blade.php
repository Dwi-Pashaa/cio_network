...
// ── TAB SWITCH ────────────────────────────────────────────────────────────────
const PROSEDUR_TABS = ['pemutusan', 'pergantian-layanan', 'onu-router', 'pergantian-password', 'rekap'];
...
.nama router baru : ${routerBaru}`;
                ...
                copyHtml = `
                <div class="message-copy-section" style="padding: 1.25rem; border-top: 1.5px solid #e2e8f0; background: #fafbfc;">
...
6.setelah melakukan langkah 1-5 segera melakuan validasi data pengajuan di bawah ini`;
            ...
            const addressParts = [];
            if (cust.hometown?.name) addressParts.push("Campung " + cust.hometown.name);
            if (cust.rt?.name) addressParts.push("RT " + cust.rt.name);
            if (cust.rw?.name) addressParts.push("RW " + cust.rw.name);
            if (cust.village?.name) addressParts.push("Desa " + cust.village.name);
            if (cust.district?.name) addressParts.push("Kec. " + cust.district.name);
...
function renderValCard(item, isPending = true) {
    ...
    if (isPending) {
        ...
        if (item.user_can_validate) {
            ...
            console.log('Spam ID:', item.id);
            console.log('Active Validation:', activeValidation);
            console.log('isOltActive:', isOltActive);
            console.log('isMixActive:', isMixActive);
            console.log('isOncActive:', isOncActive);
            console.log('isOnuRouter:', isOnuRouter);
...
function showOncSteps(id) {
    ...
    if (item.prosedur_type === 'pemutusan') {
        ...
        $('.onc-steps-body').html(`
            ...
            <p style="margin-bottom: 1rem;">
                KAMI TINGGAL MENUNGGU KONFIRMASI TAHAP AKHIR DARI ANDA (<strong class="text-dark">${authUserName}</strong>) UNTUK MELAKUKAN PERUBAHAN DATA DIBAGIAN DATA PELANGGAN SECARA OTOMATIS DIBARENGI DENGAN PENGIRIMKAN NOTIFIKASI SUKSES SECARA OTOMATIS KE TEKNISI (<strong class="text-dark">${techName}</strong>).
            </p>
            <p style="margin-bottom: 1rem;">
