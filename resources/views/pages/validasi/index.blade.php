    ...
    if (type === 'pergantian-layanan') {
        ...
        <div class="diff-block" id="diff-${item.id}" style="display: none;">
            ...
            <div class="diff-rows">
                ...
                ${p.paket_name    ? `<div class="diff-row diff-info-row"><div class="diff-label">Tipe Paket</div><div class="diff-info-val" style="color:#2563eb;font-weight:700;">${p.paket_name}</div></div>` : (p.paket_id ? `<div class="diff-row diff-info-row"><div class="diff-label">Tipe Paket</div><div class="diff-info-val">ID: ${p.paket_id}</div></div>` : '')}
                ${p.price_name    ? `<div class="diff-row diff-info-row"><div class="diff-label">Tipe Pembayaran</div><div class="diff-info-val" style="color:#2563eb;font-weight:700;">${p.price_name}</div></div>` : (p.price_id ? `<div class="diff-row diff-info-row"><div class="diff-label">Tipe Pembayaran</div><div class="diff-info-val">ID: ${p.price_id}</div></div>` : '')}
                ${p.mic_radius_name ? `<div class="diff-row diff-info-row"><div class="diff-label">Mix Radius</div><div class="diff-info-val" style="font-weight:700;">${p.mic_radius_name}</div></div>` : (p.mic_radius_id ? `<div class="diff-row diff-info-row"><div class="diff-label">Mix Radius</div><div class="diff-info-val">ID: ${p.mic_radius_id}</div></div>` : '')}
            </div>
        </div>`;
    }
