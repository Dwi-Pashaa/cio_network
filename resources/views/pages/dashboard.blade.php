            ...
            <div>
                ...
                @can('download qrcode')
                    <div style="margin-top: 1rem; display: flex; flex-wrap: wrap; gap: 0.75rem;">
                        ...
                        <button type="button" class="btn btn-dark btn-pill d-inline-flex align-items-center gap-2" id="btn-download-qrcode-reset" style="background: rgba(15, 23, 42, 0.65); border: 1px solid rgba(255,255,255,0.12); color: #fff; backdrop-filter: blur(8px); padding: 0.5rem 1.25rem; font-size: 0.825rem; font-weight: 700; transition: all 0.2s;">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M21 2v6h-6" />
                                <path d="M21 13a9 9 0 1 1-3-7.7L21 8" />
                            </svg>
                            Download QR Code Reset WiFi PPPoE
                        </button>
                    </div>
                @endcan
            </div>
...
@push('modal')
    ...
    <!-- Modal QR Code Reset -->
    <div class="modal modal-blur fade" id="modal-qrcode-reset" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 440px;" role="document">
            <div class="modal-content" style="border-radius:24px; overflow:hidden; border: none; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.35);">
                <div class="modal-header" style="background: linear-gradient(135deg,#0f172a,#1e3a8a); border:none; padding: 1.25rem 1.5rem;">
                    <h5 class="modal-title" style="color:white; font-weight:800; font-size:0.95rem; display:flex; align-items:center; gap:8px;">
                        ...
                        QR Code Reset WiFi PPPoE
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body text-center" style="padding: 1.75rem; background: #f8fafc;">
                    <p style="font-size: 0.8rem; color: #64748b; font-weight: 600; margin-bottom: 1.25rem; text-align: left;">
                        Preview Kartu QR Code Reset yang akan diunduh:
                    </p>

                    <!-- Beautiful card preview wrapper -->
                    <div id="qrcode-reset-preview-card" style="background: linear-gradient(135deg, #0f172a 0%, #1e3a8a 60%, #2563eb 100%); border-radius: 20px; padding: 2rem 1.5rem; text-align: center; border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 10px 25px rgba(30, 41, 59, 0.15); margin-bottom: 1.25rem; position: relative;">
                        <div style="font-size: 1.5rem; font-weight: 900; color: #ffffff; margin-bottom: 0.15rem; letter-spacing: 0.05em; font-family: 'Plus Jakarta Sans', sans-serif;">
                            Cio Network Solution
                        </div>
                        <div style="font-size: 0.75rem; color: rgba(255,255,255,0.7); font-weight: 600; margin-bottom: 1.5rem;">
                        ...
                        <div class="d-flex justify-content-center align-items-center" style="background: white; border-radius: 16px; padding: 0.75rem; display: inline-block; margin-bottom: 1.5rem; box-shadow: 0 8px 16px rgba(0,0,0,0.1);">
                            <div id="qrcode-reset-canvas" style="display: inline-block; background: white;"></div>
                        </div>

                        <div style="color: #ffffff; font-size: 0.875rem; font-weight: 700; margin-bottom: 0.5rem; font-family: 'Plus Jakarta Sans', sans-serif;">Pindai QR Code Untuk:</div>
                        <div style="color: rgba(255,255,255,0.85); font-size: 0.78rem; font-weight: 500; text-align: left; max-width: 250px; margin: 0 auto; line-height: 1.4; padding-bottom: 0.5rem;">
                            <div>1. Reset Password WiFi Anda</div>
                            <div>2. Reset Password PPPoE Koneksi</div>
                        </div>
                    </div>
                    
                    <div style="margin-bottom: 1.25rem; text-align: left;">
                        <label style="font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #94a3b8; display: block; margin-bottom: 0.4rem;">Public Reset Link</label>
                        <input type="text" class="form-control" style="font-size: 0.8rem; font-weight: 600; background: #ffffff; border: 1.5px solid #e2e8f0; border-radius: 10px; text-align: center; color: #475569; cursor: pointer; padding: 0.5rem;" value="{{ route('public.customer.reset_wifi') }}" readonly onclick="this.select(); document.execCommand('copy'); alertify.success('URL disalin ke clipboard!');">
                    </div>

                    <button type="button" class="btn btn-primary w-100" id="btn-save-qr-reset" style="background: linear-gradient(135deg, #2563eb, #1d4ed8); border: none; border-radius: 12px; font-weight: 700; padding: 0.75rem; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.25);">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 6px; vertical-align: middle;">
                            <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                            <polyline points="7 10 12 15 17 10" />
                            <line x1="12" y1="15" x2="12" y2="3" />
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // QR Code Reset WiFi PPPoE
            let qrCodeResetGenerated = false;
            const qrcodeResetContainer = document.getElementById("qrcode-reset-canvas");
            const publicResetUrl = "{{ route('public.customer.reset_wifi') }}";

            $('#btn-download-qrcode-reset').click(function() {
                var qrModal = new bootstrap.Modal(document.getElementById('modal-qrcode-reset'));
                qrModal.show();

                if (!qrCodeResetGenerated) {
                    new QRCode(qrcodeResetContainer, {
                        text: publicResetUrl,
                        width: 200,
                        height: 200,
                        colorDark: "#0f172a",
                    ...
                    qrCodeResetGenerated = true;
                }
            });

            $('#btn-save-qr-reset').click(function() {
                const qrCanvas = qrcodeResetContainer.querySelector('canvas');
                const qrImg = qrcodeResetContainer.querySelector('img');
                ...
                ctx.font = '600 12px "Plus Jakarta Sans", "Inter", sans-serif';
                ctx.fillStyle = 'rgba(255, 255, 255, 0.7)';
                ctx.fillText('Reset WiFi & PPPoE', 200, 75);
                ...
                function drawDetailsAndDownload() {
                    ...
                    ctx.font = '500 12px "Plus Jakarta Sans", "Inter", sans-serif';
                    ctx.fillStyle = 'rgba(255, 255, 255, 0.85)';
                    ctx.fillText('1. Reset Password WiFi Anda', 200, 395);
                    ctx.fillText('2. Reset Password PPPoE Koneksi', 200, 420);

                    // Footer URL
                    ctx.font = 'bold 11px monospace';
                    ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
                    ctx.fillText('client.cionetwork.id', 200, 470);
                    ...
                    try {
                        ...
                        const link = document.createElement('a');
                        link.href = src;
                        link.download = 'qrcode-reset-wifi-pppoe.png';
                        document.body.appendChild(link);
                        link.click();
                        document.body.removeChild(link);
                    } catch(e) {
